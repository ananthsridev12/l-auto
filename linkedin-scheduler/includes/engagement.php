<?php
// Engagement (Like & Comment) — an admin-curated list of external
// LinkedIn posts a workspace's members are encouraged to engage with.
//
// LinkedIn's Social Actions API (the only way to actually like/comment
// as a member via API) requires Community Management API partner
// approval — confirmed via a live 403 ACCESS_DENIED
// (partnerApiSocialActions.CREATE) — which this app doesn't have and
// can't get self-serve. Reposting someone else's content turned out to
// hit the same wall (a generic 403 on reshareContext, confirmed across
// both plausible URN-type guesses — see li_create_repost()'s doc
// comment in includes/linkedin_api.php) and isn't pursued further here.
// So instead, clicking Like/Comment opens the real post on LinkedIn in
// a new tab AND immediately logs the action as done. This is an
// honor-system, self-reported record, not a verified one — if the
// member doesn't actually follow through on LinkedIn, it's still
// counted. That's a deliberate, accepted tradeoff (see
// engagement_actions.verification), not an oversight.
//
// Every action is logged to engagement_actions the moment it happens —
// the full engagement record for anything done through this app, and
// the data source a future points feature will read from.
// r_member_social (LinkedIn's OWN "who engaged" read permission) isn't
// self-serve either, which is the whole reason this app logs its own
// actions rather than asking LinkedIn who did what.
//
// Requires includes/organizations.php (user_can_access_workspace()) and
// includes/linkedin_api.php (li_embed_url()) to already be loaded —
// same no-self-require convention as includes/post_helpers.php.
...
// Used to hard-block a repeat Like from the same user on the same
// target (checked server-side, not just reflected in the UI's
// disabled-button state) — matters more for a self-reported action
// than a verified one, since it has no other backstop against someone
// farming repeat "engagement" for a future points feature by clicking
// the same button twice. Comments are deliberately NOT deduped this
// way — a person can legitimately comment more than once. Generic over
// $actionType (not just 'like') so it stays reusable if another
// dedupe-worthy action type shows up later.
