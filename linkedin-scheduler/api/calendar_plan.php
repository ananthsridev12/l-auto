...
$batchStmt = $pdo->prepare(
    'INSERT INTO calendar_batches (user_id, period_days, posts_per_week, start_date, status, mix_config) VALUES (?, ?, ?, ?, "content_review", ?)'
);
$batchStmt->execute([$userId, $periodDays, $postsPerWeek, $startDate, json_encode($mixConfig)]);
$batchId = (int) $pdo->lastInsertId();

$insertPost = $pdo->prepare(
    'INSERT INTO posts (user_id, campaign_id, title, format, status, scheduled_at, calendar_batch_id, content_pillar_id, persona_id)
     VALUES (?, ?, ?, ?, "draft", ?, ?, ?, ?)'
);
$postIds = [];
foreach ($plan as $i => $slot) {
    ...
    $insertPost->execute([
        $userId, $campaignId, null, $slot['format'], $slot['date'] . ' 09:00:00',
