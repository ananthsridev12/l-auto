...
{
    $stmt = db()->prepare('SELECT id, name, bg_color, text_color, accent_color, cta_color, signature_color, body_color, accent_text_color, badge_text_color, cta_text_color, background_image_path, is_default FROM brand_palettes WHERE user_id = ? ORDER BY name');
    $stmt->execute([$userId]);
    return $stmt->fetchAll();
}
...
{
    $stmt = db()->prepare('SELECT id, name, bg_color, text_color, accent_color, cta_color, signature_color, body_color, accent_text_color, badge_text_color, cta_text_color, background_image_path, is_default FROM brand_palettes WHERE user_id = ? AND id = ?');
...
{
    $stmt = db()->prepare('SELECT id, name, bg_color, text_color, accent_color, cta_color, signature_color, body_color, accent_text_color, badge_text_color, cta_text_color, background_image_path, is_default FROM brand_palettes WHERE user_id = ? AND is_default = 1 LIMIT 1');
    $stmt->execute([$userId]);
    return $stmt->fetch() ?: null;
}
