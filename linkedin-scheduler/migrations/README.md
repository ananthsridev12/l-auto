...
| `0025_grav_taxonomy.sql` | — | New `blog_posts.grav_category`/`grav_service`/`grav_industry` — per the site's taxonomy reference doc: `taxonomy.category` (News: fixed Company/Product/Industry; Blog: free label), `taxonomy.service` (Blog/Portfolio/Case Study — the target service page's exact URL slug, drives its "Related work" cross-linking), and `industry` (Portfolio/Case Study — a plain header field, not nested under `taxonomy`). All optional; a post with none set publishes exactly as before. See `includes/grav_api.php` `grav_publish_post()`, `pages/blog_studio.php`'s Edit form. |
| `0026_news_auto_blog.sql` | — | New `workspaces.news_auto_blog_enabled`/`news_blog_drafts_per_day` — the daily news cron can now also auto-generate a Blog Studio draft from a fresh headline (separate on/off + cap from the existing LinkedIn auto-drafting; a workspace can run either, both, or neither). Always lands as a Draft for review — the cron never schedules or publishes it. See `includes/news_fetch.php` `news_generate_blog_draft()`, `cron/news_daily.php`, `pages/settings.php`'s News Auto-Content section. |
| `0027_pillar_blog_defaults.sql` | — | New `content_pillars.blog_content_type`/`blog_length`/`blog_mode`/`blog_cite_source`/`blog_fresh_context`/`grav_category` — per-pillar defaults for auto-generated blog posts (content structure, length, Original/Grounded mode, source citation, fresh-context, and the Grav taxonomy category), same NULL-means-no-override convention as `default_layout`/`default_palette`. Lets `news_generate_blog_draft()` write each pillar's content the way it's actually meant to be written instead of one fixed default for every pillar. See `pages/knowledge.php`'s Content Pillar form. |
| `0028_social_accounts.sql` | — | New `social_accounts` table (platform-agnostic connected accounts — Facebook Pages, Instagram Business, Pinterest boards, Google Business Profile locations) plus `posts.platform`/`posts.social_account_id`, additive alongside the existing LinkedIn-only `linkedin_accounts`/`posts.linkedin_account_id` (default `platform='linkedin'`, zero behavior change for existing rows). See `includes/social_publish.php`, `includes/facebook_api.php`, `includes/instagram_api.php`, `includes/pinterest_api.php`, `includes/gbp_api.php`. |
| `0029_social_external_post_id.sql` | — | New `posts.external_post_id` — a platform-neutral home for the created post/pin/localPost id returned by Facebook/Instagram/Pinterest/Google Business Profile, alongside the existing LinkedIn-specific `li_post_urn` (left untouched). See `includes/social_publish.php`. |
